...
export async function POST(
  ...
  if (!Number.isInteger(predictionId)) return redirectTo(request, '/2m/admin/pronostics');
  ...
  if ('error' in input) {
    return redirectTo(
      request,
      `/2m/admin/pronostics/${predictionId}?erreur=${encodeURIComponent(input.error)}`,
    );
  }
  ...
  return redirectTo(
    request,
    `/2m/admin/pronostics?ok=${encodeURIComponent('Pronostic mis à jour.')}`,
  );
}
