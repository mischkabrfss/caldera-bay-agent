...
export default async function VipAreaPage({
  ...
  if (!user) redirect(`/2m/connexion?erreur=${encodeURIComponent('Connectez-vous pour accéder à votre espace VIP.')}`);
      ...
      <section className="section">
        <div className="wrap stack gap-14" style={{ maxWidth: 520 }}>
          ...
          <div className="card center stack gap-12">
            ...
            <Link href={`/2m/vip#${offer.slug}`} className="btn btn-block mt-8">
              Débloquer {VIP_LABELS[vipType]}
            </Link>
            <Link href="/2m/compte" className="small">
              Retour à mon espace
            </Link>
          </div>
        </div>
        <MemberTabs active="/2m/vip" />
      </section>
    );
  }
      ...
      <section className="section">
        <div className="wrap stack gap-14">
          ...
          <div className="stack gap-10 mt-16">
            <Link href={`/2m/historique?vip=${VIP_SLUGS[vipType]}`} className="btn-line btn-block">
              Historique {VIP_LABELS[vipType]}
            </Link>
            <Link href="/2m/compte" className="btn-ghost btn-block">
              Retour à mon espace
            </Link>
          </div>
        </div>
